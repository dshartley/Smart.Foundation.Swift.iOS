//
//  PlaySpaceDataModelAdministrator.swift
//  SFGame
//
...
/// Manages PlaySpaceData data
public class PlaySpaceDataModelAdministrator: ModelAdministratorBase {
	...
	public func select(byPlaySpaceID playSpaceID: String, oncomplete completionHandler:@escaping ([Any]?, Error?) -> Void) {
		...
		// Load the data
		(self.modelAccessStrategy as! ProtocolPlaySpaceDataModelAccessStrategy).select(byPlaySpaceID: playSpaceID, collection: collection!, oncomplete: selectCompletionHandler)
	}
	
	public func toWrappers() -> [PlaySpaceDataWrapper] {
		
		var result:             [PlaySpaceDataWrapper] = [PlaySpaceDataWrapper]()
		
		if let collection = self.collection {
			
			let collection:     PlaySpaceDataCollection = collection as! PlaySpaceDataCollection
			...
			for item in collection.items! {
				...
				if (item.status != .deleted && item.status != .obsolete) {
					...
					// Get item wrapper
					let wrapper: PlaySpaceDataWrapper = (item as! PlaySpaceData).toWrapper()
	...
	public override func newCollection() -> ProtocolModelItemCollection? {
		...
		return PlaySpaceDataCollection(modelAdministrator: self,
